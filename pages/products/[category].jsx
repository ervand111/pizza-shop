...
import { filterProducts, getProductsCategories, getMoreProducts } from 'store/products/actions'; // Add getMoreProducts action
...
const Index = () => {
    ...
    const [loadingMore, setLoadingMore] = useState(false);  // Track if we're currently loading more products

    const handleScroll = (e) => {
        const bottom = e.target.scrollHeight === e.target.scrollTop + e.target.clientHeight;
        if (bottom && !loadingMore && !isFetching) {
            setLoadingMore(true);
            setPage(prevPage => {
                const nextPage = prevPage + 1;
                dispatch(getMoreProducts.request({ page: nextPage, category, ids }));
                return nextPage;
            });
        }
    };
            ...
            <CloseIcon />
              ...
              <Form onFinish={handleSubmit} form={form}>
                  <ul className={styles.filterList}>
                      {categories.map((item) => {
                          const name = locale === 'en' ? item.name_en : (locale === 'ru') ? item.name_ru : item.name;
                          return (
                            <li key={item.id}>
                                <span>{item.name}</span>
                                <span>
